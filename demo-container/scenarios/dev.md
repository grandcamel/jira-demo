...
```
Generate a git branch name for DEMO-86
```
...
```
Generate a branch name for DEMO-86 with automatic prefix based on issue type
```
...
```
Generate a hotfix branch name for DEMO-86
```

## Step 4: Get Git Checkout Command

```
Give me the git checkout command for DEMO-86
```
...
```
Generate a pull request description for DEMO-86
```
...
```
Generate a PR description for DEMO-86 with a testing checklist
```
...
```
Generate a PR description for DEMO-86 including labels and components
```

## Step 9: Link a Commit to an Issue

```
Link commit abc123 to DEMO-86 with message "fix: resolve login timeout"
```
...
```
Link PR https://github.com/org/repo/pull/456 to DEMO-86
```

## Step 11: View Linked Commits

```
Show me the commits linked to DEMO-86
```
...
```
Show detailed commit information for DEMO-86
```
...
|------------|--------|---------|
| Bug | `bugfix/` | `bugfix/DEMO-86-login-timeout` |
| Story | `feature/` | `feature/DEMO-88-user-profile` |
| Task | `feature/` | `feature/DEMO-90-add-logging` |
| Hotfix | `hotfix/` | `hotfix/DEMO-92-critical-fix` |
